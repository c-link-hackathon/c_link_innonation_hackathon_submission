    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn transfer(
            ...
            Contracts::<T>::try_mutate_exists(digest.clone(), |contract| {
                ...
                contract.status = Status::WaitingToConfirm(confirmer, new_party_b.clone(), additional);
                contract.updated = current;
        ...
        pub fn confirm(
            ...
            Contracts::<T>::try_mutate_exists(digest.clone(), |contract| {
                ...
                match &contract.status {
                    Status::WaitingToConfirm(Some(confirmer), new_party_b, additional) => {
        ...
        pub fn revoke(
            ...
            Contracts::<T>::try_mutate_exists(digest.clone(), |option_contract| {
                ...
                match &contract.status {
                    Status::WaitingToConfirm(_, _, _) if is_expired || is_owner => {
                        contract.status = Status::CanTransfer;
                        contract.updated = current;
                    },
