...
const CLAUSE: &[u8] = b"clause";
const ADDITIONAL: &[u8] = b"additional clause";
...
fn draft_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(clause, Bytes::from(CLAUSE.to_vec()));
        assert_eq!(additional, vec![].into());
    })
}
...
fn transfer_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(hash, digest.clone());
        assert_eq!(status, Status::WaitingToConfirm(Some(PARTY_B), NEW_PARTY_B, Bytes::from(ADDITIONAL.to_vec())));
        assert_eq!(updated, 1);
        assert_eq!(party_a, Some(PARTY_A));
        assert_eq!(party_b, Some(PARTY_B));
        assert_eq!(clause, Bytes::from(CLAUSE.to_vec()));
...
fn confirm_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(clause, Bytes::from(CLAUSE.to_vec()));
        assert_eq!(additional, Bytes::from(ADDITIONAL.to_vec()));
    })
}
